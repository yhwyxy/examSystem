...
- 不改复合题既有行为。
- �� `allowed_languages` 的旧代码题保持字符串答案兼容。
- 敏感字段不得下发学生端。
...
- [ ] 规范化顶层/子题 `answers_by_language`、`scoring_points_by_language`
- [ ] `sanitize_for_student` 剥离上述字段
- [ ] 测试通过
...
- [ ] `normalize_submitted_answer` 支持顶层 code + `{answer, language}`
- [ ] 评分前 `apply_language_reference(question, language)` 覆盖 answer/scoring_points/code_language
- [ ] 测试：选 js 时用 js 参考；非法语言拒绝
...
- [ ] `renderQuestion` 渲染语言 select
- [ ] `collectAnswers` 提交 `{answer, language}`
- [ ] 静态断言
...
- [ ] q16/q21 增加 `allowed_languages` 与 `answers_by_language`
- [ ] q21 改为 `scoring_mode: code`

### Task 5: 回归

- [ ] `pytest tests/test_papers.py tests/test_core.py tests/test_frontend_static.py tests/test_submission_auto_submit.py -q`
- [ ] commit
